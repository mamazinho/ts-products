...
class ProductModel(Settings.Base):
    ...
    brand = Column(String)
    status = Column(Enum(Status, names='status'), nullable=False)
    ...
    def __str__(self):
        return f"{self.id} - {self.seller_id} - {self.name} - {self.description} - {self.brand} - {self.status} - {self.created_at} - {self.updated_at} - {self.actual_stock} - {self.actual_price} - {self.gtin} - {self.active} - {[str(cat) for cat in self.categories]}"
